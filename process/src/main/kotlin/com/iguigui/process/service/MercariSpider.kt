...
class MercariSpider {
    ...
    val log = org.slf4j.LoggerFactory.getLogger(MercariSpider::class.java)
    ...
    //改名提醒
    @SubscribeBotMessage(
        functionName = "煤炉订阅", export = true, description = "" +
                "设定关键词，自动监测煤炉是否有新东西上架\n" +
                "添加订阅：格式为'煤炉订阅 关键词 是否全新 价格区间\n" +
                "如：煤炉订阅 山口百惠 非全新 200-500\n" +
                "如：煤炉订阅 中森明菜 全新 1000-5000\n" +
                "查看已订阅的列表：煤炉订阅"
    )
    suspend fun mercariSpider(dto: GroupMessagePacketDTO) {
        val contentToString = dto.contentToString()
        if (contentToString.startsWith("煤炉订阅")) {
            ...
            if (split.size == 1) {
                ...
                if (condition != "全新" && condition != "非全新") {
                    messageAdapter.sendGroupMessage(dto.sender.group.id, "是否全新格式错误")
                    return
                }
